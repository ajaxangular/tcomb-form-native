...
function datepicker(locals) {
  ...
  console.log('date picker local value');
  console.log(locals.value);
  console.log(moment(locals.value).toString());
  // console.log(moment.utc(locals.value));
  const momentObj = moment.utc(locals.value).toObject()
  console.log('moment obj');
  console.log(momentObj);
  // var editedValue = moment.utc(locals.value).toISOString();
  var editedValue = new Date(momentObj.years, momentObj.months, momentObj.date);
      ...
      <TouchableNativeFeedback
        ...
        onPress={function () {
          if (datePickerMode === 'time') {
            ...
            DatePickerAndroid.open({date: editedValue || new Date()})
              .then(function (date) {
                if (date.action !== DatePickerAndroid.dismissedAction) {
                  // var newDate = new Date(date.year, date.month, date.day);
                  var newDate = new Date(Date.UTC(date.year, date.month, date.day));
                  // var newDateObj = moment.utc(date).toObject();
                  // var newDate = new Date(newDateObj.years, newDateObj.months, newDateObj.date);
                  // var newDate = new Date(Date.UTC(newDateObj.years, newDateObj.months, newDateObj.date));
